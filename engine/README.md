...
**전에 여기 "지도가 아직 좋지 않다" 고 적혀 있었다 — 원인은 시퀀스가 아니라
우리가 뽑은 간격이었다.** 그때는 `--stride 3` 으로 40 장을 뽑아 프레임당 약
1.4 m 였고, WME 의 ATE 가 **1251 cm** 였다 (같은 구간 ORB + PnP 는 31.58 cm).
`--stride 1` 로 **같은 궤적(원본 440~557)을 118 장**으로 다시 받아 재니
같은 추정기가 **11.88 cm** 로 내려오고 ORB + PnP(14.42 cm)를 이긴다.

간격을 3 으로 뽑은 근거는 "프레임당 1.4 m 면 KITTI 의 1.5 m 와 같은 자리"
였는데, 그 기준이 틀렸다. 직접 광도 정합이 감당하는 것은 미터가 아니라
**화소 변위**이고 그것은 장면 깊이와 프레임당 회전이 정한다. 이 크롭은
KITTI 보다 훨씬 가깝고(중앙 깊이 3~6 m vs `kitti_00` 10.4 m) 훨씬 빨리 돈다
— 프레임당 회전이 `kitti_04` 0.22°, `kitti_00` 1.49° 인데 여기는 3.38° 였다.
그래서 미터를 맞췄더니 회전이 만드는 광류만 20.1 px 로 `kitti_00` 의 3 배가
됐고, 앞 열한 프레임에서 정합이 참 이동의 4~70 % 밖에 잡지 못했다
(추정 0.05 m vs 참 1.32 m). 판정은 같은 구간에서 **끼워 넣은 두 장 말고는
완전히 같은 자료**로 했다 — 겹치는 12 자리의 화소·포즈·시각이 비트까지 같다.

깊이·보정·좌표계는 원인이 아니었다. 같은 프레임에서 ORB + PnP 가 2.70 cm 로
멀쩡했기 때문이다 — 자료가 아니라 **직접 정합의 수렴 반경**이 걸린 것이다.
