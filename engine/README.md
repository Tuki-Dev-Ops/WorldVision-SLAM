...
내보낸 GLB 는 규격에 맞는지 검사했다 - 청크 길이, 선언한 버퍼 크기 대 실제
청크, 모든 접근자의 count 대 bufferView 바이트 길이. JSON 은 파싱과 좌표
변환을 재현해 값의 범위를 확인했다 (kitti_00 201 프레임: 건물 237 · 나무 56 ·
기둥 1 · 차량 46, 237 m x 112 m 범위, 치수가 말이 안 되는 건물 0 개).

**두 임포터 스크립트는 이 저장소에서 실행해 보지 못했다.** Unity 도 Unreal 도
여기 설치되어 있지 않다. 문법과 좌표 변환은 확인했지만, 실제 에디터에서
돌려 본 것은 아니다.
