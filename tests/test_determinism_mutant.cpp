...
TEST(MutantControl, TokenOrderDifferenceIsDetected) {
    ...
    // 여기까지 왔다면 시도한 모든 배치에서 주소 순서와 id 순서가 일치했다는
    // 뜻이다. 그런 힙에서는 이 변이가 **의미상 무해** 하므로 관측으로 구분할
    // 방법이 없다 - 검출 실패가 아니라 검출할 것이 없는 상태다.
    //
    // ASan 의 할당자가 정확히 그렇다: 크기 클래스별로 순차 할당하므로 같은
    // 타입의 객체는 만든 순서 = 주소 순서가 되고, 토큰은 id 순서대로 만들어진다.
    //
    // 그래서 실패가 아니라 skip 이다. 다만 **왜** skip 인지 남긴다 - 이유 없는
    // skip 은 초록으로 보이는 침묵이고, 이 저장소가 반복해 기록한 실패 양식이다.
    GTEST_SKIP() << "시도한 모든 힙 배치에서 주소 순서 == id 순서였다. "
                    "그런 할당자(ASan 등)에서는 이 변이가 관측 가능한 차이를 "
                    "만들지 않으므로 검출할 대상이 없다. 다른 할당자에서는 "
                    "검출된다 - MSVC/glibc 에서 pad 0 으로 통과 확인.";
}
