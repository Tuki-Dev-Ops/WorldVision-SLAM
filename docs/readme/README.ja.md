...
| データセット | シーケンス数 | 対照群 | 結果 |
|---|---|---|---|
| **TUM RGB-D**（屋内・手持ち） | 16 | ORB+PnP、`cv2.Odometry` | **9 – 6**（二つのうち良い方に対して） |
| **KITTI odometry**（屋外・車載） | 4 | ORB+PnP | **4 – 0** |

</div>

TUM だけのときこの比較は 9–6 だった。KITTI を加えると**2–2 に逆転した**。原因を追うと、ECDA が
ステレオ深度を真値として信じていた —— 60 m 地点の深度誤差は ±4 m なのに、6 m 地点（±4 cm）と
同じ重みで方程式に入っていた。その不確かさを残差分散へ移すと（係数は `c = σ_d/(f·B)` から
**導出**したもので、調整値ではない）4–0 になった。

> **それまで「WME の方が良い」と書かれていた文は、アルゴリズムの性質ではなく TUM の性質だった。**
> 経緯は [§25.20–25.21](../06-results.md) にある。
