...
| 数据集 | 序列数 | 对照组 | 结果（单一配置） |
|---|---|---|---|
| **TUM RGB-D**（室内，手持） | 16 | ORB+PnP、`cv2.Odometry` | **8 – 7**（对两者中较强的一方） |
| **KITTI odometry**（室外，车载） | 4 | ORB+PnP | **4 – 0** |

</div>

**TUM 实际上是平局。** 若逐序列挑选更好的 WME 变体，结果是 10–5；但那是已部署系统做不到的选择。
上表对所有序列使用**同一套配置**（Tier 0）。

这些数字变动过两次，两次的原因都在测量一侧，而不在算法。

- **加入 KITTI 后翻转为 2–2。** ECDA 把双目深度当作真值 —— 60 m 处的深度误差是 ±4 m，却与
  6 m 处（±4 cm）以相同权重进入方程。把这份不确定性移入残差方差之后（系数由
  `c = σ_d/(f·B)` **推导**而来，并非调参），结果变为 4–0。（[§25.20–25.21](../06-results.md)）
- **16 个 TUM 序列中有 13 个只按其中一部分帧计分** —— 平均 35 %，最低 6.4 %。解压中断后帧索引
  完好而帧文件缺失，于是每个加载器只读取存在的文件，并报告一次干净的运行。重新下载并重跑之后，
  五个判定发生翻转 —— 且是**双向的**。（[§25.22](../06-results.md)）

> **此前所有写着"WME 更好"的句子，一次是 TUM 的性质而非算法的性质，一次是只看了三分之一数据得出的结果。**
